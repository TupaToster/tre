...
void bufferize (const char* filename, Text* writeTo) {
    ...
    writeTo->str = (char*) calloc (writeTo->TextSize + 2, sizeof (char));
    assert (writeTo->str != NULL);
    ...
    assert (fread (writeTo->str, sizeof (char), writeTo->TextSize, fIn) != 0);

    writeTo->str[writeTo->TextSize] = '\0';
...
void separateLines (Text* writeTo) {
    ...
    for (size_t i = 0; i < writeTo->TextSize; i++) {
        ...
        if (writeTo->Lines[Line_iter].begin == NULL) writeTo->Lines[Line_iter].begin = writeTo->str + i;

        if (writeTo->str[i] == '\n') {

            writeTo->Lines[Line_iter].end = writeTo->str + i + 1;
            Line_iter++;
        }
    }

    writeTo->Lines[Line_iter].end = writeTo->str + writeTo->TextSize;
}